...
{
    /// <summary>
    ///     TERA's SHA-1 implementation. Close to the original SHA-1 algorithm, but with two differences: expanded values
    ///     aren't rotated and the output DWORDs are little-endian.
    /// </summary>
    internal class SHA1Fake : SHA1
    {
        ...
        byte[] _EndHash()
        {
            ...
            /* Store digest */
            //Difference from original SHA-1: DWORDs are encoded as little-endian.
            //DWORDToBigEndian(hash, _stateSHA1, 5);
            DWORDToLittleEndian(hash, _stateSHA1, 5);

            //HashValue = hash;
            return hash;
        }

        //Modified version of System.Security.Cryptography.Utils.DWORDToBigEndian
        ...
        private static unsafe void DWORDFromBigEndian(uint* x, int digits, byte* block)
        {
            ...
            while (index1 < digits)
            {
                x[index1] = (uint)(block[index2] << 24 | block[index2 + 1] << 16 | block[index2 + 2] << 8) |
                            block[index2 + 3];
                ++index1;
                index2 += 4;
            }
        ...
        static unsafe void SHAExpand(uint* x)
        {
            ...
            for (i = 16; i < 80; i++)
            {
                ...
                //Difference from original SHA-1: expanded values aren't rotated.
                //x[i] = ((tmp << 1) | (tmp >> 31));
                x[i] = tmp;
            }
        }
